    ...
    def split_data_region(self):
        ...
        with open(DATASET_PATH, encoding='utf-8') as file_stream:    
            ...
            for row in file_stream_reader:
                region = row['Region']
                # Open a new file and write the header
                if path.exists(CURATED_LOCAL_PATH + REGION) == False:
                    try:
                        os.makedirs(CURATED_LOCAL_PATH + REGION)
                    except OSError:
                        print ("Creation of the directory %s failed" % path)
                        exit(1)

                if region not in open_files_references:
                    output_file = open(CURATED_LOCAL_PATH + REGION +'{}.csv'.format(region), 'w', encoding='utf-8', newline='')
                    dictionary_writer = csv.DictWriter(output_file, fieldnames=file_stream_reader.fieldnames)
                    dictionary_writer.writeheader()
